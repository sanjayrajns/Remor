...
export default function StarredView({ onOpenQuickCapture }) {
  const { getFilteredItems } = useStore();
  const items = getFilteredItems().filter(i => i.starred);
    ...
    <div style={{ flex: 1, padding: '24px 32px', overflowY: 'auto' }}>
      <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px' }}>
        <div>
          ...
          <p className="text-secondary text-sm" style={{ marginTop: '4px' }}>
            Your bookmarked, high-value, and frequently referenced knowledge assets.
          </p>
        </div>
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Click the star icon on any link, note, task, or idea to keep it pinned for quick access."
        ...
        <div>
          <div className="text-xs font-mono text-muted font-600" style={{ marginBottom: '16px' }}>
            IMPORTANT &amp; BOOKMARKED ({items.length})
          </div>
